...
The span of the column vectors of $A$. It lives in $\mathbb{R}^m$.
...
### 2. Null Space, $N(A)$
The set of all vectors $\mathbf{x}$ that $A$ crushes entirely to zero.
...
The span of the rows of $A$ (lives in $\mathbb{R}^n$). 

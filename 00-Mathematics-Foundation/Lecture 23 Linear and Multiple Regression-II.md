...
# 1. Load Data
df = pd.read_csv('student_scores.csv') # 25 samples
...
# 3. Extract the Math
print(f"Intercept (Beta 0): {reg.intercept_}")
print(f"Slope (Beta 1): {reg.coef_[0]}")
```
...
```python
# 1. Prepare Data
# X contains 3 columns (R&D, Admin, Marketing)
# y contains the Profit
